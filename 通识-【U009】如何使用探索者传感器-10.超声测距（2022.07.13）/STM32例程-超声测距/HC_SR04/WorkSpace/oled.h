...
#define OLED_IIC_SDA_Read()   GPIO_ReadInputDataBit(GPIOA,GPIO_Pin_5)

#define OLED_IIC_SDA_Out() 	{GPIOA->MODER&=~(3<<(5*2));GPIOA->MODER|=1<<(5*2);} //PC9输出模式
#define OLED_IIC_SDA_In()	{GPIOA->MODER&=~(3<<(5*2));GPIOA->MODER|=0<<(5*2);}	//PC9输入模式
...
void show1(void* param);
void show2(void* param);
