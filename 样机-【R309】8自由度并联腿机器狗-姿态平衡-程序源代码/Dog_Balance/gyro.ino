...
void DispData(Packet_t *pkt)
{
    ...
    {
        memcpy(AccRaw, &(pkt->buf[1]), 6);
    }

    if (pkt->buf[7] == kItemGyoRaw) /* gyro raw value */
    {
        memcpy(GyoRaw, &(pkt->buf[8]), 6);
    }

    //  if(pkt->buf[14] == kItemMagRaw)  /* mag raw value */
    //  {
    //    memcpy(MagRaw, pkt->buf, 6);
    //  }
    if (pkt->buf[14] == kItemAtdE) /* atd E */
    {
        Eular[0] = ((float)(int16_t)(pkt->buf[15] + (pkt->buf[16] << 8))) / 100;
        Eular[1] = ((float)(int16_t)(pkt->buf[17] + (pkt->buf[18] << 8))) / 100;
        Eular[2] = ((float)(int16_t)(pkt->buf[19] + (pkt->buf[20] << 8))) / 10;
    }
}

void readPose()
{
    /* 如果软件串口接收到姿态模块数据，则处理并显示 */
