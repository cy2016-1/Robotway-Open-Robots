...
def solve():
    ...
    try:
        #defstr = txt_face.get(1.0, END).rstrip() + '\n'         #文本框
        #defstr = get_definition_string() + '\n'
        total_color.deal_color()
        #print(total_color.output)
        defstr = total_color.output+'\n'
...
canvas.pack()
carGif=PhotoImage(file="E:\Python_project\gxu.gif")
...
hp_window = canvas.create_window(10 + 0 * width, 0 + .5 * width, anchor=NW, window=hp)
fa = Label(text='               Face')
fa_window = canvas.create_window(10 + 0 * width, 0 + 1.7 * width, anchor=NW, window=fa)
txt_host = Text(height=1, width=20)
txt_host_window = canvas.create_window(10 + 0 * width, 0 + .9 * width, anchor=NW, window=txt_host)
txt_host.insert(INSERT, DEFAULT_HOST)
txt_port = Text(height=1, width=20)
txt_port_window = canvas.create_window(10 + 0 * width, 0 + 1.3 * width, anchor=NW, window=txt_port)
txt_port.insert(INSERT, DEFAULT_PORT)
txt_face = Text(height=3, width=20)
txt_face_window = canvas.create_window(10 + 0 * width, 5 + 2.0 * width, anchor=NW, window=txt_face)
